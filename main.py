...
import os
from fastapi import FastAPI, Request, WebSocket, WebSocketDisconnect # Added WebSocket imports
...
from typing import Dict # Added for connection manager type hinting
...
class ConnectionManager:
    ...
    def get_state(self, websocket: WebSocket) -> dict:
        return self.active_connections.get(websocket, {"last_question": None, "last_word": None})
...
async def websocket_endpoint(websocket: WebSocket):
    ...
    if dspy.settings.lm:
        ...
        if question_data and question_data.get("question_viability"):
            ...
            manager.update_state(websocket, "last_question", client_state['last_question'])
            manager.update_state(websocket, "last_word", client_state['last_word'])
            print(f"WS {websocket.client}: Stored initial question state: Word='{client_state['last_word']}'")
            initial_data_payload["question"] = question_data.get("question")
            initial_data_payload["feedback"] = question_data.get("feedback") # Optional: maybe send later
            initial_data_payload["usage_sentences"] = question_data.get("usage_sentences")
        else:
             print(f"WS {websocket.client}: Initial question not viable or error generating.")
             initial_data_payload["question"] = question_data.get("question", "Error generating initial question.")
             initial_data_payload["feedback"] = question_data.get("feedback", "Check server logs.")
             # No state to store
    else:
        print(f"WS {websocket.client}: Skipping initial question generation - LLM not configured.")
    ...
    await manager.send_personal_message({"type": "initial", "payload": initial_data_payload}, websocket)
    print(f"WS {websocket.client}: Sent initial data.")

    # --- Main Loop to Handle Incoming Messages ---
    try:
        while True:
            raw_data = await websocket.receive_text() # Or receive_json if client always sends JSON
            ...
            try:
                ...
                if message_type != "chat" or not user_message:
                    await manager.send_personal_message({"type": "error", "payload": "Invalid message format."}, websocket)
                    continue

            except json.JSONDecodeError:
                 await manager.send_personal_message({"type": "error", "payload": "Invalid JSON received."}, websocket)
                 print(f"WS {websocket.client}: Invalid JSON received: {raw_data}")
                 continue
            except Exception as e:
                 await manager.send_personal_message({"type": "error", "payload": f"Error processing message: {e}"}, websocket)
                 print(f"WS {websocket.client}: Error processing message: {e}")
                 continue


            print(f"WS {websocket.client}: Message from user: {user_message}")
            client_state = manager.get_state(websocket) # Get current state for this client
            ...
            if not dspy.settings.lm:
                print(f"WS {websocket.client}: Skipping chat processing - LLM not configured.")
                ...
                if last_question and last_word:
                    print(f"WS {websocket.client}: Expecting answer for word '{last_word}'. Assessing...")
                    ...
                    print(f"WS {websocket.client}: Cleared question state after assessment.")

                else:
                    print(f"WS {websocket.client}: Not expecting an answer. Generating standard reply or new question...")
                    ...
                    if "Okay, here's a new vocabulary question:" in ai_response_text:
                        print(f"WS {websocket.client}: generate_ai_reply triggered a new question. Attempting to store state...")
                        ...
                        if new_question_data and new_question_data.get("question_viability"):
                            manager.update_state(websocket, "last_question", new_question_data.get("question"))
                            manager.update_state(websocket, "last_word", new_question_data.get("challenging_word"))
                            print(f"WS {websocket.client}: Stored NEW question state from generate_ai_reply: Word='{manager.get_state(websocket).get('last_word')}'")
                        else:
                             print(f"WS {websocket.client}: Failed to store state for new question generated by generate_ai_reply.")
        ...
        try:
             await manager.send_personal_message({"type": "error", "payload": f"Server error: {e}. Disconnecting."}, websocket)
        except Exception:
...
async def get_initial_passage_with_question_http():
    ...
    # WARNING: This uses a simple global state, not suitable for multiple users.
    # It's kept here primarily for comparison or testing.
    global_session_state = {} # Temporary local dict for this request simulating old behavior
...
async def chat_endpoint_http(user_input: UserInput):
    ...
    # WARNING: Uses simulated global state. Not recommended for production.
    print(f"--- HTTP POST /chat_http received request ---")
    global_session_state = {} # Simulate state, don't actually use a real shared global
    ...
    print(f"--- HTTP Endpoint /chat_http finishing request ---")
    return ChatResponse(response=ai_response_text)
