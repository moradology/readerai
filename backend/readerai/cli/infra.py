...
"""
    ...
    if run_terraform_command(cmd, infra_dir) == 0:
        ...
        console.print("Run 'readerai infra apply' to apply changes")
    else:
...
def apply(
    ...
    auto_approve: bool = typer.Option(
        False, "--auto-approve", help="Skip confirmation"
    ),
    environment: str = typer.Option(
        None, "--env", "-e", help="Environment (dev/staging/prod)"
    ),
    ...
    console.print("[bold blue]Applying infrastructure changes...[/bold blue]")
    ...
    if plan_file:
        ...
        if not plan_path.exists():
            console.print(f"[red]Error: Plan file '{plan_file}' not found[/red]")
            raise typer.Exit(1)
        cmd.append(plan_file)
    else:
        # Direct apply without plan file
        if environment:
    ...
    if run_terraform_command(cmd, infra_dir) == 0:
        ...
        if result.returncode == 0:
            ...
            env_content = f"""# Generated by readerai infra apply
