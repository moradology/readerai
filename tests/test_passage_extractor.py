...
def test_text_chunker_legacy():
    ...
    assert total_chars - overlap_chars <= len(preprocessed) + 100, "Chunks should cover the whole text"
...
def test_text_chunker_streaming():
    ...
    assert len(stream_chunks) == len(legacy_chunks), "Stream and legacy methods should produce same number of chunks"
    
    # Compare content (allowing for minor differences due to implementation details)
    for i, (stream_chunk, legacy_chunk) in enumerate(zip(stream_chunks, legacy_chunks)):
        # Check if chunks are identical or very similar in length
        length_diff = abs(len(stream_chunk) - len(legacy_chunk))
        assert length_diff <= 50, f"Chunk {i} differs significantly in size between methods"
...
def test_chunker_error_handling():
    ...
    assert sum(len(chunk) for chunk in long_chunks) >= len(long_line), "Should process entire input"
...
"""
    ...
    assert len(stream_chunks) > 0, "Should extract at least one chunk"
    assert "Test content." in stream_chunks[0], "Should preserve content in streaming mode"
    assert "Project Gutenberg" not in stream_chunks[0], "Should remove header in streaming mode"
