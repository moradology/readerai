...
def process_stream(
    ...
    try:
        for chunk in chunker.chunk_stream(input_stream):
            ...
            if verbose:
                print(
                    f"Processing chunk {chunk_count} ({len(chunk)} chars, {total_chars_processed} total)...",
                    file=sys.stderr,
                )
                if elapsed > 0:
                    print(
                        f"  Rate: {total_chars_processed / elapsed:.2f} chars/sec",
                        file=sys.stderr,
                    )
            else:
                print(
                    f"Processing chunk {chunk_count} ({len(chunk)} chars)...",
                    file=sys.stderr,
                )
            ...
            try:
                ...
                for passage in result.passages:
                    ...
                    passage["content"] = clean_text(
                        passage["content"], clean_chars, strip_formatting
                    )
                    passage["title"] = clean_text(
                        passage["title"], clean_chars, strip_formatting
                    )

                    # Write as JSONL, one passage per line
                    try:
                        output_stream.write(
                            json.dumps(passage, ensure_ascii=True) + "\n"
                        )
                        ...
                        if verbose:
                            print(
                                f"  Extracted passage: {passage['title']}",
                                file=sys.stderr,
                            )
                        elif passage_count % 10 == 0:
                            print(
                                f"Extracted {passage_count} passages so far...",
                                file=sys.stderr,
                            )
                    except OSError as e:
    ...
    elapsed = time.time() - start_time
    print(
        f"\nProcessing complete: {chunk_count} chunks, {passage_count} passages extracted",
        file=sys.stderr,
    )
    if elapsed > 0:
        print(f"Total processing time: {elapsed:.2f} seconds", file=sys.stderr)
        print(
            f"Average processing rate: {total_chars_processed / elapsed:.2f} chars/sec",
            file=sys.stderr,
        )
        if passage_count > 0:
            print(
                f"Average passage size: {total_chars_processed / passage_count:.2f} chars/passage",
                file=sys.stderr,
            )
...
def main() -> int:
    ...
    parser.add_argument(
        "--output_file", type=str, help="Output file path (defaults to stdout)"
    )
    parser.add_argument(
        "--min_paragraphs", type=int, default=3, help="Minimum paragraphs per passage"
    )
    parser.add_argument(
        "--max_paragraphs", type=int, default=7, help="Maximum paragraphs per passage"
    )
    parser.add_argument(
        "--validate", action="store_true", help="Perform quality validation"
    )
    parser.add_argument(
        "--no-validate",
        dest="validate",
    ...
    parser.add_argument(
        "--chunk_overlap", type=int, default=2000, help="Chunk overlap in characters"
    )
    parser.add_argument(
        "--verbose", "-v", action="store_true", help="Enable verbose progress reporting"
    )
    parser.add_argument(
        "--model",
        type=str,
    ...
    parser.set_defaults(
        validate=True, verbose=False, clean_text=True, strip_formatting=False
    )
    ...
    print(
        f"Starting stream processing from stdin using model: {args.model}...",
        file=sys.stderr,
    )
    if args.verbose:
        print(
            ...
            f"\n  - Quality validation: {args.validate}",
            file=sys.stderr,
        )
    ...
    chunker = TextChunker(
        max_chunk_size=args.max_chunk_size, overlap=args.chunk_overlap
    )
    extractor = PassageExtractorFlow()
    ...
    try:
        ...
        if args.output_file and not args.verbose:
            print(
                f"Wrote {passage_count} passages to {args.output_file}", file=sys.stderr
            )
