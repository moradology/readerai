...
# Make sure VocabularyFlow and AssessStudentAnswer are correctly imported or defined
try:
    from .vocabulary import VocabularyFlow, AssessStudentAnswer
    import dspy
except ImportError:
    # Fallback implementations (keep as before)
    class VocabularyFlow: # Mock
        def __call__(self, passage):
            class MockPrediction:
                question = "Mock Question: What does 'subterranean' mean?"
                feedback = "Mock Feedback: Check context clues."
                challenging_word = "subterranean"
                usage_sentences = "Mock usage sentences."
                question_viability = True
            return MockPrediction()
    class AssessStudentAnswer(dspy.Signature): # Mock
...
# --- Import the passage from the constants file ---
from ..constants import TEST_PASSAGE # Use relative import from parent directory
...
def generate_vocab_question_data(passage_text: str) -> dict | None:
    ...
    if not dspy.settings.lm:
         ...
         return {"question": "Error: LLM not configured.", "feedback": "Please check server logs."}
    try:
        ...
        if not prediction.get('question_viability', False):
             return {
                 "question": prediction.get('question', 'Could not generate viable question.'),
...
def assess_student_answer(passage_text: str, question_asked: str, word_asked: str, student_answer: str) -> dict | None:
    """
    ...
    try:
        ...
        return {
            ...
            "assessment_feedback": assessment.get('assessment_feedback', 'Could not get assessment feedback.')
        }
    except Exception as e:
...
def generate_ai_reply(user_message: str) -> str:
    ...
    # ... (other simple handlers) ...
        ...
        return "Goodbye!"
    elif "vocabulary question" in lower_message or "vocab question" in lower_message or "next question" in lower_message:
        # --- Use the imported TEST_PASSAGE when user explicitly asks ---
        question_data = generate_vocab_question_data(TEST_PASSAGE) # Use imported constant
        if question_data and question_data.get("question_viability"):
            ...
            error_feedback = question_data.get('feedback', 'Could not generate question.') if question_data else 'Could not generate question.'
