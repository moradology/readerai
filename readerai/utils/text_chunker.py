...
class TextChunker:
    ...
    def _find_paragraph_boundary(
        ...
        """
        # Handle invalid inputs which could cause hanging
        if not text:
            return 0

        if position < 0 or position > len(text):
            # Safety check to prevent out-of-bounds position
            position = min(max(0, position), len(text))

        # Handle the case where there's no newline in the text
        # This is likely the root cause of hanging in the long line test
        if "\n" not in text:
            # If no newlines at all, return either beginning or end depending on direction
            return len(text) if direction == "forward" else 0
    ...
    def preprocess_chunk(
        self, text: str, is_first_chunk: bool = False, is_last_chunk: bool = False
    ) -> str:
        """
    ...
    def chunk_stream(self, input_stream: TextIO) -> Iterator[str]:
        ...
        while True:
            ...
            if chunk_count > 0 and chunk_count % 5 == 0:
                ...
                if (
                    hasattr(input_stream, "tell")
                    and hasattr(input_stream, "seekable")
                    and input_stream.seekable()
                ):
                    try:
                        print(
                            f"Processing stream at position {input_stream.tell()} bytes, "
                            f"chunk {chunk_count}...",
                            file=sys.stderr,
                        )
                    except OSError:
                        ...
                        print(
                            f"Processing stream, chunk {chunk_count}...",
                            file=sys.stderr,
                        )
                else:
            ...
            read_count = 0
            max_lines_per_iteration = 10000  # Safety limit to prevent infinite loops

            while (
                len(buffer) < self.max_chunk_size
                and not eof
                and read_count < max_lines_per_iteration
            ):
                try:
                    ...
                    if len(line) > max_line_length:
                        print(
                            f"Warning: Extremely long line detected ({len(line)} chars). "
                            f"Breaking into smaller segments.",
                            file=sys.stderr,
                        )

                        # Break extremely long line into smaller segments by inserting
                        # artificial paragraph breaks every max_line_length chars
                        for i in range(0, len(line), max_line_length):
                            segment = line[i : i + max_line_length]
                            buffer += segment
                            # Add artificial newlines for very long lines to ensure
                            # we can find paragraph boundaries
                            if i + max_line_length < len(line):
                                buffer += "\n\n"
                    else:
                        buffer += line
                    ...
                    print(
                        f"Warning: Error reading input: {e}. Skipping problematic data.",
                        file=sys.stderr,
                    )
                    # Try to recover by reading the next line
                    continue

            # Safety check - if we hit the max_lines_per_iteration, process what we have
            if read_count >= max_lines_per_iteration and not eof:
                print(
                    f"Warning: Reached maximum lines per iteration ({max_lines_per_iteration}). "
                    f"Processing current buffer to prevent infinite loop.",
                    file=sys.stderr,
                )
            ...
            if len(buffer) <= self.max_chunk_size and eof:
                ...
                chunk = self.preprocess_chunk(
                    buffer, is_first_chunk=(chunk_count == 0), is_last_chunk=True
                )
                if chunk:
                    yield chunk
                break
            ...
            # Force a maximum size in case we can't find a good boundary
            # This prevents getting stuck with extremely long text without natural boundaries
            max_possible_size = min(len(buffer), self.max_chunk_size * 2)

            if end_pos < len(buffer):
                boundary_pos = self._find_paragraph_boundary(
                    buffer, end_pos, "backward"
                )

                # If boundary found is too close to the start or not found at all,
                # force a reasonable chunk size to ensure progress
                if boundary_pos < self.max_chunk_size / 4:
                    # If the boundary is very close to the beginning, it's not a good split point
                    # Force a split at a reasonable position
                    end_pos = min(self.max_chunk_size, len(buffer))
                else:
                    end_pos = boundary_pos

            # Ensure we don't exceed max size even if no boundary is found
            end_pos = min(end_pos, max_possible_size)

            # Extract the chunk
            chunk_text = buffer[:end_pos]
            preprocessed_chunk = self.preprocess_chunk(
                chunk_text, is_first_chunk=(chunk_count == 0), is_last_chunk=False
            )
            ...
            # Capture buffer length before update to detect progress
            old_buffer_length = len(buffer)

            # Update buffer for next iteration
            buffer = buffer[overlap_start:]

            # Safety check: if buffer isn't changing or is empty, break
            if not buffer:
                break

            # Ensure we're making progress - prevent infinite loop
            if len(buffer) >= old_buffer_length:
                print(
                    "Warning: Buffer not shrinking in next iteration. "
                    "Forcing progress by removing first 1000 chars to prevent infinite loop.",
                    file=sys.stderr,
                )

                # Force some progress - remove some content from the front
                if len(buffer) > 1000:
                    buffer = buffer[1000:]
                else:
                    # If buffer is small but not shrinking, we're stuck
                    # Just process the rest as one chunk and exit
                    if buffer:
                        final_chunk = self.preprocess_chunk(
                            buffer, is_first_chunk=False, is_last_chunk=True
                        )
                        if final_chunk:
                            yield final_chunk
                    break
...
def test_chunker():
    ...
    for i, chunk in enumerate(chunks):
        print(
            f"  Chunk {i + 1}: {len(chunk)} chars, {chunk.count('Paragraph')} paragraphs"
        )
    ...
    for chunk in chunker.chunk_stream(text_stream):
        ...
        print(
            f"  Stream chunk {chunk_count}: {len(chunk)} chars, {chunk.count('Paragraph')} paragraphs"
        )
